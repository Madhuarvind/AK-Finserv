...
class RiskPredictor:
    ...
    def _extract_features(customer_id, loan):
        ...
        features['payment_rate'] = (paid_emis / total_emis * 100) if total_emis > 0 else 100
        
        # 3. Days Since Last Payment
        last_collection = Collection.query.filter_by(
            loan_id=loan.id, 
            status='approved'
        ...
        if emis:
            ...
            features['partial_payment_ratio'] = (partial_count / len(partial_payments) * 100) if partial_payments else 0
        else:
            features['partial_payment_ratio'] = 0
        
        # 5. Loan Utilization (pending amount vs total payable)
        if loan.pending_amount > 0:
            total_payable = sum(e.amount for e in emis) if emis else loan.principal_amount
            features['utilization_pct'] = (loan.pending_amount / total_payable * 100) if total_payable > 0 else 0
        else:
            features['utilization_pct'] = 0
        
        # 6. Tenure Progress (how far into the loan)
        if emis:
            paid_emis_count = EMISchedule.query.filter_by(loan_id=loan.id, status='paid').count()
    ...
    def get_portfolio_risk_overview():
        ...
        active_customers = db.session.query(Loan.customer_id).filter(
            Loan.status == 'active'
        ).distinct().all()

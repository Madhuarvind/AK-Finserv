...
def get_customer_risk_score(customer_id):
    ...
    last_collection = Collection.query.filter_by(loan_id=active_loan.id, status='approved')\
    ...
    avg_emi_amount = db.session.query(func.avg(EMISchedule.amount)).filter_by(loan_id=active_loan.id).scalar() or 0
    recent_collections = Collection.query.filter_by(loan_id=active_loan.id, status='approved')\
                        .order_by(Collection.created_at.desc()).limit(5).all()
    
    partial_pattern_score = 0
    if recent_collections:
        smaller_payments = [c for c in recent_collections if c.amount < (avg_emi_amount * 0.9)]
        if len(smaller_payments) >= 3: # Constant partial payments is a sign of cash flow issues
    ...
    if days_since_last_payment > 20:
        insights.append(f"No payment received in the last {days_since_last_payment} days.")
...
def get_worker_performance_analytics():
    ...
    for agent in agents:
        ...
        # Risk factors
        flagged_count = Collection.query.filter_by(agent_id=agent.id, status='flagged').count()
        
        # Batch entry detection
        timestamp_counts = db.session.query(Collection.created_at)\
                            .filter_by(agent_id=agent.id)\
                            .group_by(Collection.created_at)\
                            .having(func.count(Collection.id) > 1).all()
    ...
    for data in raw_data:
        ...
        if data['batch_events'] > 2:
             risk_flags.append(f"High frequency batch entries detected: {data['batch_events']}")
        ...
        display_score = 75
        if cluster == "TOP PERFORMER": display_score = 95
        if cluster == "UNDERPERFORMING": display_score = 45
        if is_suspicious: display_score = 20
    ...
    analytics.sort(key=lambda x: (x['cluster'] == 'SUSPICIOUS ACTIVITY', x['performance_score']), reverse=True)
...
def get_customer_behavior_analytics(customer_id):
    ...
    observations = []
    if segment == "VIP (GOLD)":
        observations.append("ML Cluster: Top Tier Customer. High priority for retention.")
    elif segment == "HIGH RISK":
        observations.append("ML Cluster: High Risk. Strict collection monitoring advised.")
    elif segment == "SILVER":
...
def _calculate_customer_stats(customer, loans):
    ...
    for loan in loans:
        ...
        for emi in emis:
            if emi.status == 'paid':
                ...
                # Check delay
                last_coll = Collection.query.filter_by(loan_id=loan.id, status='approved').order_by(Collection.created_at.desc()).first()
                if last_coll:
    ...
    reliability = (total_paid_on_time / total_emis_due * 100) if total_emis_due > 0 else 100
    ...
    avg_capacity = sum(payment_amounts) / len(payment_amounts) if payment_amounts else 5000
    ...
    return {
        ...
        "total_loans_closed": len([l for l in loans if l.status == 'closed']),
...
def get_dashboard_ai_insights():
    ...
    this_week_total = db.session.query(func.sum(Collection.amount)).filter(
        Collection.created_at >= last_week, Collection.status == 'approved'
    ).scalar() or 0
    
    prev_week_total = db.session.query(func.sum(Collection.amount)).filter(
        Collection.created_at >= prev_week, Collection.created_at < last_week, Collection.status == 'approved'
    ).scalar() or 0
    
    collection_drop_pct = 0
    if prev_week_total > 0:
        collection_drop_pct = ((prev_week_total - this_week_total) / prev_week_total) * 100
    ...
    if collection_drop_pct > 5:
        ...
        for ag in agents:
            ...
            amt = db.session.query(func.sum(Collection.amount)).filter_by(agent_id=ag.id).scalar() or 0
            agent_data.append({'agent_id': ag.id, 'amount': amt, 'count': colls, 'batch_events': 0, 'flagged': 0})
    ...
    for loan in active_loans:
        ...
        missed = EMISchedule.query.filter(EMISchedule.loan_id == loan.id, EMISchedule.status != 'paid', EMISchedule.due_date < today).count()
        last_pay = Collection.query.filter_by(loan_id=loan.id).order_by(Collection.created_at.desc()).first()
        days_since = (today - last_pay.created_at).days if last_pay else 30
        
        prob, level = risk_engine.predict_risk(missed, 30, days_since, 0) # simplified features
    ...
    risky_areas = [{"area": a[0], "overdue_count": a[1], "overdue_amount": 0} for a in sorted_areas] # keeping schema compatible
    ...
    problem_loans = db.session.query(
        Loan.loan_id, Customer.name, func.count(EMISchedule.id).label('missed'), Loan.pending_amount
    ).join(Customer).join(EMISchedule).filter(EMISchedule.status != 'paid', EMISchedule.due_date < today)\
     .group_by(Loan.id, Customer.name).order_by(func.count(EMISchedule.id).desc()).limit(5).all()
    ...
    if collection_drop_pct > 10:
        ...
        summaries.append(f"Positive growth: Weekly collections are up by {round(abs(collection_drop_pct))}%!")
        
    if risky_areas:
        top_area = risky_areas[0]
        summaries.append(f"AI Risk Heatmap: '{top_area['area']}' has the highest concentration of High-Risk borrowers ({top_area['overdue_count']} identified).")
        
    if not problem_loans:
        summaries.append("System Health: No critical loan defaults detected this week.")
    else:
        summaries.append(f"Attention: {len(problem_loans)} loans flagged for immediate collection follow-up.")

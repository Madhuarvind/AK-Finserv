...
from models import db, User, Customer, Loan, Collection, EMISchedule, UserRole, LoanAuditLog, LoginLog
...
def export_audit_csv():
    ...
    si = io.StringIO()
    cw = csv.writer(si)
    cw.writerow(['ID', 'Loan ID', 'Action', 'Performed By', 'Old Status', 'New Status', 'Timestamp', 'Remarks'])
...
def detect_tampering():
    ...
    for loan in active_loans:
        ...
        total_collected = db.session.query(db.func.sum(Collection.amount))\
                            .filter_by(loan_id=loan.id, status='approved').scalar() or 0
...
def role_abuse_detection():
    ...
    abuse_logs = db.session.query(
        LoanAuditLog.performed_by, 
        db.func.count(LoanAuditLog.id)
    ).filter(LoanAuditLog.timestamp >= one_hour_ago)\
     .group_by(LoanAuditLog.performed_by)\
     .having(db.func.count(LoanAuditLog.id) > 20).all() # More than 20 audit logs in 1 hour
...
def device_monitoring():
    ...
    multi_login = db.session.query(
        LoginLog.user_id, 
        db.func.count(db.func.distinct(LoginLog.device_info))
    ).filter(LoginLog.login_time >= recent)\
     .group_by(LoginLog.user_id)\
     .having(db.func.count(db.func.distinct(LoginLog.device_info)) > 2).all()

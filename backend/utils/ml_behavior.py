...
class BehaviorEngine:
    ...
    def analyze_behavior(self, customer_features):
        ...
        if n_clusters > 1: # Require at least 2 points for clustering
            ...
            cluster_rank = df.groupby('cluster')['reliability_score'].mean().sort_values(ascending=False)
            
            # Assign Labels
            labels = {}
            for i, (cid, score) in enumerate(cluster_rank.items()):
                if i == 0: labels[cid] = "VIP (GOLD)"
                elif i == 1: labels[cid] = "SILVER"
                elif i == 2: labels[cid] = "BRONZE"
                else: labels[cid] = "HIGH RISK"
            ...
            if rel_score >= 90: df['segment'] = "VIP (GOLD)"
            elif rel_score >= 70: df['segment'] = "SILVER"
            elif rel_score >= 40: df['segment'] = "BRONZE"
            else: df['segment'] = "HIGH RISK"
        ...
        for index, row in df.iterrows():
            ...
            suggested_limit = round(suggested_limit / 5000) * 5000 # Round to nearest 5k
            suggested_limit = max(min(suggested_limit, 200000), 10000) # Cap between 10k and 2lakh

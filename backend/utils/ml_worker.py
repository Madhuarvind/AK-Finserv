...
class WorkerIntelligence:
    ...
    def analyze_workforce(self, agent_data_list):
        ...
        if len(df) >= 10: # Increased threshold for meaningful Isolation Forest
            ...
            df['is_suspicious'] = (df['batch_events'] >= 3) | (df['flagged'] >= 2) | \
                                 ((df['amount'] == 0) & (df['count'] > 0)) # Flag zero collection attempts if activity exists
        ...
        if n_clusters > 0:
            ...
            cluster_centers = df.groupby('cluster_id')['amount'].mean().sort_values(ascending=False)

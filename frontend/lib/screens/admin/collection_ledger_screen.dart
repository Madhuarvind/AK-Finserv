          ...
          child: ListTile(
            leading: Column(
              ...
              children: [
                ...
                Text(collection['payment_mode'].toString().toUpperCase(), style: const TextStyle(fontSize: 8)),
              ],
            ),
            title: Text(collection['customer_name'], style: const TextStyle(fontWeight: FontWeight.bold)),
            subtitle: Text("${collection['agent_name']} • ${collection['loan_id']}"),
            trailing: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              crossAxisAlignment: CrossAxisAlignment.end,
              children: [
                Text("₹${collection['amount']}", style: GoogleFonts.outfit(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.green)),
                Text(_formatTime(collection['time']), style: const TextStyle(fontSize: 10, color: Colors.grey)),
              ],
            ),
          ),
        );
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          ...
          child: ListTile(
            ...
            title: Text(target['customer_name'], style: const TextStyle(fontWeight: FontWeight.bold)),
            subtitle: Text("${target['area']} • ${target['agent_name']}"),
            trailing: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              crossAxisAlignment: CrossAxisAlignment.end,
              children: [
                Text("₹${target['amount_due']}", style: GoogleFonts.outfit(fontSize: 16, fontWeight: FontWeight.bold)),
